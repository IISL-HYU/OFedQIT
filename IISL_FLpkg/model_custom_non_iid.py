...
class CustomModelList(list):
    def fed_avg_non_iid(self, x, y, x_test, y_test, metric, central_server):
      trainable_vars = central_server.model.trainable_variables
      loss_avg = 0
      sca_metric_avg = 0
      for i, model in enumerate(self):
        train_results = model.train_step_non_iid(x[int(i*len(x)/len(self)):int((i+1)*len(x)/len(self))],y[int(i*len(x)/len(self)):int((i+1)*len(x)/len(self))], x_test, y_test, metric)
    ...
    def pfed_avg_non_iid(self, x, y, x_test, y_test, metric, central_server, L, marker):
      trainable_vars = central_server.model.trainable_variables
      loss_avg = 0
      sca_metric_avg = 0
      for i, model in enumerate(self):
        train_results = model.p_train_step_non_iid(x[int(i*len(x)/len(self)):int((i+1)*len(x)/len(self))],y[int(i*len(x)/len(self)):int((i+1)*len(x)/len(self))], x_test, y_test, metric, central_server)
    ...
    def rpfed_avg_non_iid(self, x, y, x_test, y_test, metric, rp_central_server, prob, L, marker):
      trainable_vars = rp_central_server.model.trainable_variables
      loss_avg = 0
      sca_metric_avg = 0
      for i, model in enumerate(self):
        train_results = model.p_train_step_non_iid(x[int(i*len(x)/len(self)):int((i+1)*len(x)/len(self))],y[int(i*len(x)/len(self)):int((i+1)*len(x)/len(self))], x_test, y_test, metric, rp_central_server)
        model.optimizer.apply_gradients(zip(train_results[0], self[i].model.trainable_variables))
        loss_avg += train_results[1]
        sca_metric_avg += train_results[2]
        if marker % L == 0 :
...
class CustomModel(keras.Model):
    ...
    def train_step_non_iid(self, x, y, x_test, y_test, metric):
      loss_fn = keras.losses.SparseCategoricalCrossentropy()
        # Unpack the data. Its structure depends on your model and
        # on what you pass to `fit()`.
      with tf.GradientTape() as tape:
        y_pred = self.model(x, training=True)  # Forward pass
        y_test_pred = self.model(x_test, training=True)
      ...
      # # Return a dict mapping metric names to current value
      return gradients, loss_test.numpy(), result_metric
    
    def p_train_step_non_iid(self, x, y, x_test, y_test, metric, central_server):
      loss_fn = keras.losses.SparseCategoricalCrossentropy()
        # Unpack the data. Its structure depends on your model and
        # on what you pass to `fit()`.
      with tf.GradientTape() as tape:
        y_pred = self.model(x, training=True)  # Forward pass
        y_pred_cet = central_server.model(x_test, training=True)
        # Compute the loss value
        # (the loss function is configured in `compile()`)
        loss = loss_fn(y, y_pred)
        loss_cet = loss_fn(y_test, y_pred_cet)
      # Compute gradients
      trainable_vars = self.model.trainable_variables
      gradients = tape.gradient(loss, trainable_vars)
      # Update weights
      # Update metrics (includes the metric that tracks the loss)
      result_metric = 0
      metric.update_state(y_test, y_pred_cet)
      result_metric = metric.result().numpy()
      # # Return a dict mapping metric names to current value
